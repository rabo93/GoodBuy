...
public class RsaKeyGenerator {
	...
	public static Map<String, String> generateKey() {
		Map<String, String> rsaKey = new HashMap<String, String>(); // 한쌍의 키를 저장할 Map 객체
		
		try {
			// 1. 공개키/개인키 한 쌍을 생성하는 KeyPairGenerator
			// => KeyPairGenerator 클래스의 static 메서드 getInstance() 호출 (알고리즘 전달)
			KeyPairGenerator keyPairGenerator;
				keyPairGenerator = KeyPairGenerator.getInstance(CIPHER_ALGORITHM);
			
			// 2. 생성된 KeyPairGenerator 객체의 initialize() 메서드 호출하여 키의 길이(정수) 전달
			// => 현재 사용하는 일반적인 키의 길이는 2048 바이트
			keyPairGenerator.initialize(2048);
			
			// KeyPairGenerator 객체의 genKeyPair() 메서드를 호출하여 KeyPair 객체를 얻어온 후
			// KeyPair 클래스를 사용하여 PublicKey(공개키)와 Private(개인키) 객체를 생성
			KeyPair keyPair = keyPairGenerator.genKeyPair();
			PublicKey publicKey = keyPair.getPublic(); // 공개키
			PrivateKey privateKey = keyPair.getPrivate(); // 개인키
					
			// Base64 인코딩을 사용하여 생성된 키를 문자열로 변환
