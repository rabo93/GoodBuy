							 	...
							 	<div class="account-box">
							 		<div class="search-header">
										...
										<div class="search-date">
											<span>조회기간 : </span> 
											<input type="date" placeholder="시작" class="start_date" id="start_date" value="${startDate}"> 
											~ <input type="date" placeholder="끝" class="end_date" id="end_date" value="${endDate}">
											<button class="date-search-btn" onclick="searchDate()">조회</button>
										</div> 							                
							        </div>
						            ...
						            <section id="pageList">
						            	<input type="button" value="&lt;&lt;" 
											onclick="location.href='AllPayList?pageNum=${pageInfo.startPage - pageInfo.pageListLimit}<c:if test="${not empty param.start_date}">&start_date=${startDate}</c:if><c:if test="${not empty param.start_date}">&end_date=${endDate}</c:if>'"
											<c:if test="${pageInfo.startPage eq 1}">disabled</c:if>
										>
										<input type="button" value="이전" 
											onclick="location.href='AllPayList?pageNum=${pageNum - 1}<c:if test="${not empty param.start_date}">&start_date=${startDate}</c:if><c:if test="${not empty param.start_date}">&end_date=${endDate}</c:if>'"
...
<!-- 										startPage 부터 endPage 까지 1씩 증가하면서 페이지번호 표시 -->
										<c:forEach var="i" begin="${pageInfo.startPage}" end="${pageInfo.endPage}">
											<c:choose>
												...
												<c:otherwise>
													<a href="AllPayList?pageNum=${i}<c:if test="${not empty param.start_date}">&start_date=${startDate}</c:if><c:if test="${not empty param.start_date}">&end_date=${endDate}</c:if>">${i}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach>
